...
// Add the import for TodayShowChecker
...
export default function Dashboard() {
  ...
  const upcomingByDate = upcomingEpisodes.reduce(
    (acc, episode) => {
      const date = new Date(episode.next_episode_date).toLocaleDateString()
      if (!acc[date]) {
        acc[date] = []
      }
  ...
  // Sort dates
  const sortedDates = Object.keys(upcomingByDate).sort((a, b) => new Date(a).getTime() - new Date(b).getTime())
